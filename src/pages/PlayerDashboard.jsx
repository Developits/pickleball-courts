...
export default function PlayerDashboard() {
  ...
  const handleGeolocationCheckIn = async () => {
    ...
    try {
      ...
      if (err.code === 1) {
        setMessage("❌ Location access denied. Please allow location permissions or use the QR scanner instead.");
      } else if (err.code === 2) {
        setMessage("❌ Unable to get your location. Please check your GPS and try again, or use the QR scanner.");
      } else if (err.code === 3) {
        setMessage("❌ Location request timed out. Please try again or use the QR scanner.");
      } else {
        setMessage("❌ Error checking in. Please try again or use the QR scanner.");
      }
    } finally {
  ...
  return (
    <div>
      ...
      {/* Court Status Banner */}
      <div className={`mb-4 sm:mb-6 p-4 sm:p-6 rounded-xl border-2 ${isCourtOpen ? "bg-gradient-to-r from-green-50 to-green-100 border-green-300" : "bg-gradient-to-r from-gray-50 to-gray-100 border-gray-300"}`}>
        <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
          <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center ${isCourtOpen ? "bg-green-500" : "bg-gray-500"}`}>
          ...
          <div className="flex-1">
            <h2 className={`text-lg sm:text-xl font-bold ${isCourtOpen ? "text-green-800" : "text-gray-800"}`}>
        ...
        {isCourtOpen && !checkedIn && (
          <button
            ...
            className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 disabled:opacity-50"
          >
            {isCheckingIn ? (
        ...
        {isQueueLocked && (
          <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-700 text-sm sm:text-base">
              ⚠️ Queue is locked - court closing soon! No new players can join the queue.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            {isCourtOpen && (
              <p className="text-gray-600 text-sm sm:text-base">
                Check in using your location (primary), or scan a QR code (fallback).
              </p>
            )}
